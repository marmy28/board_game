...
class CheapPlayer(Person):
    ...
    def burnCard(self):
	  ...
	  def getNeighborColor(self):
		    max_color = 'green'
		    max_length = 0
		    for color in self.neighbor['left'].cards_played.keys():
			    if color != 'wonder':
				    if len(self.neighbor['left'].cards_played[color]) > max_length:
					    max_length = len(self.neighbor['left'].cards_played[color])
					    max_color = color
		    return max_color
